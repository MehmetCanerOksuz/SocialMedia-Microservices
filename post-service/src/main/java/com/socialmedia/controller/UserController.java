...
public class UserController {
    ...
    public ResponseEntity<List<Post>> findByStatus(@PathVariable EStatus status){
        return ResponseEntity.ok(userService.findByStatus(status));
    }

    @GetMapping("/find_by_status2/{status}")
    public ResponseEntity<List<Post>> findByStatus2(@PathVariable String status){
    ...
    public ResponseEntity<Page<Post>> findAllByPageable(int pageSize, int pageNumber
    ...
    public ResponseEntity<Slice<Post>> findAllBySlice(int pageSize, int pageNumber
            , @RequestParam(required = false,defaultValue = "ASC") String direction
            , @RequestParam(required = false, defaultValue = "id") String sortParameter){
